...
public class CasellaElettronica extends UnicastRemoteObject implements CasellaElettronicaBase {
	...
	public void addMail(Mail mail) {
		Mail m = new Mail(this.mailCounter.get(), mail.data, mail.mittente, mail.destinatari, mail.priorita, mail.argomento, mail.testo);
		this.mail.add(m);
	...
	public void deleteMail(int id) throws RemoteException, CasellaElettronicaException {
		for(Mail m : this.mail)
			if(m.id == id) {
				this.mail.remove(m);
				fireRequestPerformed(new CasellaElettronicaEvent(this, CasellaElettronicaEvent.Code.REMOVE_MAIL));
				return;
			}
